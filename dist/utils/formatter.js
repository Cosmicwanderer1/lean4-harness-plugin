...
export class LeanFormatter {
     ...
     */
    static parseDiagnostics(output) {
     ...
     */
    static parseTacticState(rawState) {
        if (ObjectUtils.isEmpty(rawState))
        ...
        for (const line of lines) {
            ...
            if (targetIndex >= 0) {
                if (ObjectUtils.isEmpty(current))
                    current = { caseName: "", context: [], target: "" };
                const goal = current;
                goal.target = line.slice(targetIndex + 1).trim();
                continue;
            }
            if (!ObjectUtils.isEmpty(current) && line.trim() !== "")
     ...
     */
    static formatTacticStateMarkdown(state) {
        if (state.goals.length === 0)
            return "已完成证明，当前没有待证明目标。";
     ...
     * @returns {string} Markdown 文本。
     */
    static formatDiagnosticsMarkdown(diagnostics) {
