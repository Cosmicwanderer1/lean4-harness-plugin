...
- LeanCopilot 属于建议提供方，不属于证明验证器。
- 通过独立接口接入 LeanCopilot，例如 `LeanSuggestionProvider`；核心服务只依赖抽象接口。
- 不在仓库中虚构 LeanCopilot 的具体 HTTP、命令行或 SDK 参数。真实协议确定后，再将其写入适配器和配置类型。
- 建议请求至少应包含源码、当前文件、导入环境、局部上下文、目标列表和 Lean 工具链版本。
- 建议结果至少应包含候选代码、目标标识、解释或置信信息、提供方版本和原始响应摘要。
- 候选建议必须通过独立的 `lean_check` 或 REPL 会话验证后，才能标记为 `accepted` 或 `verified`。
...
- 请求 ID 和会话 ID。
- Lean、Lake、Mathlib（如果使用）和 LeanCopilot 版本。
- 输入源码或源码摘要。
...
LeanCopilot 接入分为三个阶段：

### 阶段一：建议适配器

- 增加 `LeanSuggestionProvider` 类型和默认空实现。
- 定义统一的建议请求和建议响应结构。
- 支持超时、取消、服务不可用和响应解析失败。
- 让 `lean_check` 在失败后可以选择性地产生建议，但默认不强制调用外部建议服务。

### 阶段二：上下文增强

- 从 Lean REPL 或语言服务器获取精确 tactic state。
- 提取当前目标、局部变量、相关声明、导入模块和源码位置。
- 建立候选建议与目标 ID、源码范围之间的映射。
- 支持一次返回多个候选，并限制候选数量、代码长度和执行时间。

### 阶段三：自动修正闭环

- 模型生成初始代码。
- Lean 验证代码并返回诊断。
- LeanCopilot 根据诊断和 tactic state 生成候选修正。
- 插件将候选逐一放入隔离验证会话。
- 只把通过 Lean 验证的候选反馈为可靠结果，其余候选标记为未验证。
- 记录每次尝试，支持最大迭代次数、总耗时和成本上限。

建议闭环必须支持人工或宿主侧关闭，不能默认无限自动修正，也不能在没有新验证结果时重复调用建议服务。

## 6. 计划中的服务与工具

在保持现有工具兼容的前提下，后续可以增加：

- `lean_verify_project`：验证整个 Lake 项目，而不只是临时源码文件。
- `lean_get_diagnostics`：获取指定会话或文件的结构化诊断。
- `lean_get_tactic_state`：获取当前证明目标和局部上下文。
- `lean_suggest_tactics`：调用 LeanCopilot 或其他建议提供方生成候选 tactic。
- `lean_verify_suggestion`：对单个候选建议执行独立验证。
- `lean_repair`：在受控迭代次数内完成“诊断 -> 建议 -> 验证”闭环。

工具设计要求：输入和输出结构稳定、字段含义明确、错误可以区分、结果可以被模型直接消费。新增工具必须同步更新 `README.md`、`src/types/index.ts`、测试和开发进度文档。
...
不要在测试中依赖真实 LeanCopilot 线上服务。外部建议服务应使用确定性的本地模拟适配器；必要时增加明确标记的集成测试。
...
- 不把宿主 SDK、LeanCopilot、Mathlib 的未确认 API 写成事实；通过适配器和配置隔离外部依赖。
...
- 不将未经过 Lean 验证的模型或 LeanCopilot 输出标记为证明成功。
